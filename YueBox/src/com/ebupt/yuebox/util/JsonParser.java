...
public class JsonParser {
	public static TimeStamp parseTimeSamp(String result) throws JSONException {
		...
		return new TimeStamp(jSon.getLong("WOTimeStamp"),
				jSon.getLong("TSTimeStamp"));
	}

	public static List<SetupTask> parseSetupTask(String result)
		...
		for (int i = 0; i < data.length(); i++) {
			...
			int lng, lat;
			if (jSon.getString("task_client_lng").equals("")
					|| jSon.getString("task_client_lat").equals(""))
				continue;
	...
	public static List<AppSetupUser> parseAppSetupUser(String result)
			throws JSONException {
		List<AppSetupUser> appSetupUsers = new ArrayList<AppSetupUser>();
		JSONArray data = new JSONArray(result);
		for (int i = 0; i < data.length(); i++) {
			JSONObject jSon = data.getJSONObject(i);
			AppSetupUser appSetupUser = new AppSetupUser(
					jSon.getString("user_id"), jSon.getString("user_name"),
					null, jSon.getString("user_tel"),
					jSon.getString("user_email"),
					jSon.getString("user_name_spell"),
					jSon.getString("user_job"), jSon.getString("user_grade"),
					jSon.getString("user_department"),
					jSon.getInt("user_total_credits"),
					jSon.getInt("user_year_credits"),
					jSon.getInt("user_month_credits"),
					jSon.getInt("user_week_credits"), null, null, null, null);
